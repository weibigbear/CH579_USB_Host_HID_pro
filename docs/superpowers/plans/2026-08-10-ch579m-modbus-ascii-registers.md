...
- [x] **Step 1 实施结果（2026-08-10 实测，USB-TTL 直连 PA4/PA5，PA6 悬空）**

键盘 → 寄存器 → Modbus 应答全链路验证通过，实测日志摘录：

1. **键盘写寄存器 + 0x03 应答**：敲 `1 2 3` + Enter 后发 `01 03 00 00 00 05 85 C9` → 应答 `01 03 0A 00 31 00 32 00 33 00 00 00 00 6A E0`（40001~3 = '1''2''3'，其后 0，CRC 正确）
2. **空闲超时自动提交**：敲 `a s d` 不按 Enter，停顿 13 秒后读 → 应答 `01 03 0A 00 61 00 73 00 64 00 00 00 00 B2 2B`（500ms 超时提交生效）
3. **Backspace 删字**：敲 `1 2 3` → Backspace → Enter → 读 → 应答 `01 03 0A 00 31 00 32 00 00 00 00 00 00 6E E4`（40003 已清零）
4. **分段读 125**：发 `01 03 00 00 00 7D 85 EB` → 应答 `01 03 FA 00 31 00 32 00 00 ...`（253 字节长帧正常）
5. **非法功能码**：发 `01 04 00 00 00 05 30 09` → 应答异常帧 `01 84 01 82 C0`（0x04|0x80 + 异常码 0x01）
6. **错误 CRC**：发 `01 03 00 00 00 05 00 00` → 无应答（静默丢弃）
7. **错误地址**：发 `02 03 00 00 00 05 85 FA` → 无应答（非本机地址不响应）

结论：TTL 阶段全部验收通过，代码无需改动。待 RS485 硬件就绪后执行 Step 2/3（RS485 联调 + PA6 方向时序示波器验证）。
