 ...
 *           覆盖式写入不清空, 主站按 0x0088 长度+0x0089 帧号读取有效帧)
 *   配置区  0x0080 = 从机地址(1~247)   0x0081 = 波特率索引(0~4)   [0x06 可写]
 *           0x0086 = 空闲超时提交 ms(0=禁用自动提交, ≤60000)      [0x06 可写]
 *   状态区  0x0082 = 上次复位原因       0x0083 = 已处理帧数
 *           0x0084 = CRC 错误帧数(含过短帧)  0x0085 = 按键事件丢弃计数
 *           0x0088 = 待读帧长度(定帧后>0)  0x0089 = 帧号(定帧+1, 变化=新帧)
 *           0x0083~0x0085/0x0088/0x0089 为 16 位(高字节在前)      [只读, 0x03 读]
 *
 * 上层调用约定:
...
#define MODBUS_CFG_IDLE_REG 0x0086      /* 0x06 写: 空闲超时提交 ms(0=禁用, ≤60000) */
...
#define MODBUS_STAT_PENDLEN_REG 0x0088  /* 0x03 读: 待读帧长度(16 位, 定帧后>0, 新输入=0) */
#define MODBUS_STAT_FRAMENO_REG 0x0089  /* 0x03 读: 帧号(16 位, 每次定帧+1, 变化=新帧) */
