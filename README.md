...
- **按住自动连发**：按住可打印字符键（字母/数字/符号/空格）≈800ms 后按 ~50ms 间隔持续重复，模拟 PC 键盘连发（`KBD_REPEAT_DELAY_CNT` / `KBD_REPEAT_INTERVAL_CNT` 可调）；Enter/Backspace/CapsLock/修饰键刻意不重复（Enter 重复会导致帧反复提交清空，Backspace 重复会误删整帧）；**防幽灵连发三重保险**：报告快照校验（目标键不在当前报告即停）+ 确认有效期超时（~144ms 未见该键强制停）+ 合成次数上限 20 次
- **帧管理（帧号 + 长度 + 数据区）**：定帧（Enter/空闲超时）时记录帧长度（0x0088）并使帧号 +1（0x0089）；数据区覆盖式写入不清空——主站比对帧号变化识别新帧、按长度读取有效段，无残留污染、无 125 分段丢尾部问题
...
- **Modbus RTU 从机**：键盘输入字符按顺序写入保持寄存器 40001~40128（低字节 ASCII、高字节 0），9600 8N1，支持 0x03 读（数据区/配置区/状态区）与 0x06 写配置（地址/波特率/空闲超时）
...
**Modbus 参数**：从机地址 1、波特率 9600、8 数据位 / 无校验 / 1 停止位（8N1），支持功能码 `0x03` 读、`0x06` 写配置，单次读取数量 ≤125（超出需分段）。

**寄存器映射：**

| 地址 | 寄存器号 | 方向 | 内容 |
| ---- | ---- | ---- | ---- |
| 0x0000 ~ 0x007F | 40001 ~ 40128 | 只读 | 键盘 ASCII 数据区（覆盖式，按长度读） |
| 0x0080 | 40129 | 读/写 | 从机地址（1~247） |
| 0x0081 | 40130 | 读/写 | 波特率索引（0=9600, 1=19200, 2=38400, 3=57600, 4=115200） |
| 0x0086 | 40135 | 读/写 | 空闲超时自动提交 ms（0=禁用，≤60000） |
| 0x0082 ~ 0x0085 | 40131 ~ 40134 | 只读 | 状态区：复位原因 / 已处理帧数 / CRC 错误 / 按键丢弃 |
...
| Enter（主键盘 0x28 / 小键盘 0x58） | 立即定帧（余段清零，写入位复位，帧号+1） |
| Backspace | 回退一位并清零该寄存器 |
| 暂停输入（默认 1000ms，0x0086 可配） | 空闲超时自动定帧 |
| 其他键（F1、Esc、方向键等） | 忽略，不写入 |

**推荐读取流程（帧号 + 长度）：**

```
① 读 0x0088（长度 N）+ 0x0089（帧号 F）
② 帧号 F 与上次不同 → 有新帧 → 读 40001 起 N 个寄存器 → 取前 N 个字符即完整一帧
   帧号相同 → 无新帧 → 忽略
```

数据区**读后不清空**（覆盖式写入）：按长度 N 读取只取有效段，旧帧残留不可见；输入中途读取（0x0088=0）读到的是旧帧数据，主站应以"帧号变化"为准。

### 错误处理

- 非 0x03/0x06 功能码 → 异常码 `0x01`（非法功能）
- 数量为 0、>125 或地址越界 → 异常码 `0x02`（非法数据地址）
- 0x06 写非法值（地址/波特率/空闲超时越界）→ 异常码 `0x03`（非法数据值）
- 从机地址不匹配 / CRC 错误 → 不应答
...
2. 输入 `ABC` → 停 1.5 秒定帧 → 读 0x0088 = `00 03`（长度 3）、0x0089 = 帧号递增
3. 读 40001~40003 = `0x0041 / 0x0042 / 0x0043`，40004 起为 0
4. 按 Enter → 立即定帧（帧号+1），余段清零
5. 输入字符后停止 1000ms → 空闲超时自动定帧
6. 按 Backspace → 上一个字符变为 0
7. 分段读取验证：读 40001~40125（125 个）与 40126~40128（3 个），分别发包，尾部不丢
8. 错误注入：错 CRC/错地址 → 无响应；数量=0 或 >125 → 异常码 0x02；非 0x03/0x06 功能码 → 异常码 0x01
